...
function changeState( newState ) {
	...
	switch( state ) {
		case STATE_START:
			...
			chrome.contextMenus.update( 'Revoke', {
				visible: false
			} );
			chrome.contextMenus.update( 'SendToSheet', {
				visible: false
			} );
			chrome.contextMenus.update( 'GoToSheet', {
				visible: false
			} );
			break;
...
function getAuthTokenCallback( token ) {
	if( chrome.runtime.lastError ) {
		...
		chrome.contextMenus.update( 'Go2Sheet', {
			visible: true
		} );
...
chrome.contextMenus.onClicked.addListener( function( item, tab ) {
	...
	for( var c = 0; c < theTitles.length; c++ ) {
		...
		if( theField == dis ) {
			...
			chrome.contextMenus.update( theField, {
