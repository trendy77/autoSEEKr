...
var getBackNGo=(function () {
	var signin,revoke_button,binchk,close,upIds,revokeT,goButton,signinT;
	var exec_info_div,exec_data,binres,exec_result,exec_info;
	var tpl,sht,fld,;
	// a promise...
	...
	function processFields(page) {
		...
		binres=document.querySelector('#binRes');
		binres.innerText+=
	...
	var lookup=require('../../node_modules/binlookup/index.js');

	function checkIt(theBIN) {
		lookup(theBIN,function (err,data) {
			if(err) {
				return err
			}
			return data
	...
	return {
		onload: function () {
			...
			on('overlay');
			binchk=document.querySelector('#BIN');
			binchk.addEventListener('click',checkIt.bind(binchk,true));
			binres=document.querySelector('#binRes');
